...
LOG_DIR=${LOG_DIR:-${PRESERVED_ROOT}/logs/p5_cogvideo_day1}
...
(
  ...
  python scripts/smoke_test_cosmos_pipeline.py \
    --vision-model-name cogvideo_2b_pool4_m4 \
    --device cuda \
    --sample-index 0 \
...
bash scripts/launch_p5_cogvideo_precompute.sh 2>&1 | tee "${LOG_DIR}/precompute.log"
bash scripts/launch_p5_cogvideo_screen.sh 2>&1 | tee "${LOG_DIR}/screen.log"
